...
type currency struct {
	...
	fxBuying        string `xml:"fxBuying"`
	fxSelling       string `xml:"fxSelling"`
	...
	if err != nil {
		...
		if resp.StatusCode != http.StatusNotFound {
			...
			for i, curr := range tarih.Currency {
				...
				currDay.Currencies[i].fxBuying, err = strconv.ParseFloat(curr.fxBuying, 64)
				currDay.Currencies[i].fxSelling, err = strconv.ParseFloat(curr.fxSelling, 64)
			...
			fmt.Println(currDay.Currencies[1].CurrencyNameTR + " / " + strconv.FormatFloat(currDay.Currencies[1].BanknoteSelling, 'E', -1, 64))
